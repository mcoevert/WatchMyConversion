...
function sendTowatchMyConversion(conversionId, label, value) {
  ...
  UrlFetchApp.fetch('https://europe-west1-watch-my-conversion.cloudfunctions.net/updateConversion?api_key=tXXLtWp3Yz6K7t3m', options)
  return value;
}
...
function getVisitors(profileId, start, end) {
  var startDate = Utilities.formatDate(start, Session.getScriptTimeZone(),
      'yyyy-MM-dd');
  var endDate = Utilities.formatDate(end, Session.getScriptTimeZone(),
      'yyyy-MM-dd');
